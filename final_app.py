...
def transform_data(df):
    ...
    text_set = " ".join(yes_df["Comment"].astype(str))
    ...
    grouped_data_bar = df_words.groupby('word')['count'].sum()
    top_10_data = grouped_data_bar.sort_values(ascending=False).head(10)
    top10 = np.flip(top_10_data)
    
    return (no_df, yes_df, df_words, df_gb_gender, df_gb_age, df_gb_sentiment, df_gb_status, yes_df_gb_gender_sentiment, yes_df_gb_age_sentiment, 
            count_surveys, count_yes, df_sum, yes_df_sum, df_point_age, df_point_gender, df_count_status, df_count_sentiment, 
            yes_count_gender_sentiment, yes_count_age_sentiment, table_compare, top10)

#-----------------------------------------------------------------------------------#

### Initialize Dash app
app = dash.Dash(external_stylesheets=[dbc.themes.MINTY], title='Mayson Place')
...
### Callback of updated charts
@app.callback(
    [
        Output('op_count_age', 'figure'),
        # Output('op_count_gender', 'figure'),
        # Output('op_stat_point', 'figure'),
        # Output('op_age_mean', 'figure'),
        # Output('op_gender_mean', 'figure'),
        
        # Output('cp_have_count', 'figure'),
        # Output('cp_pn_count', 'figure'),
        # Output('cp_pn_count_gender', 'figure'),
        # Output('cp_pn_count_age', 'figure'),
        # Output('cp_compare_avg_point', 'figure'),
        # Output('cp_compare_std_point', 'figure'),

    #  Output('wp_word_cloud', 'figure'),
...
def update_charts(start_date, end_date):
    ...
    (no_df, yes_df,df_words, df_gb_gender, df_gb_age, df_gb_sentiment, df_gb_status, yes_df_gb_gender_sentiment, yes_df_gb_age_sentiment, 
    count_surveys, count_yes, df_sum, yes_df_sum, df_point_age, df_point_gender, df_count_status, df_count_sentiment, 
    yes_count_gender_sentiment, yes_count_age_sentiment, table_compare, top10) = transform_data(filtered_df)

    op_count_age_fig = create_bar_chart_ng(df_point_age, x='Age_Range', y='count',title='Count by age range',x_label='Age range', y_label='# of surveys',color=minty_colors['primary'])
    op_count_gender = create_bar_chart_ng(df_point_gender, x='Gender', y='count',title='Count by gender',x_label='Gender', y_label='# of surveys',color=minty_colors['secondary'])
    op_stat_point = create_bar_chart_ng(df_sum, x='index', y='Mean_Point',title='Statistics on satisfaction point',x_label='Stat', y_label='Satisfaction point',color=minty_colors['accent'])
    op_age_mean = create_bar_chart_ng(df_point_age, x='Age_Range', y='mean',title='Satisfaction point by age range',x_label='Age range', y_label='Satisfaction point',color=minty_colors['primary'])
    op_gender_mean = create_bar_chart_ng(df_point_gender, x='Gender', y='mean', title='Satisfaction point by gender',x_label='Gender', y_label='Satisfaction point',color=minty_colors['secondary'])

    cp_have_count = create_pie_chart(df_count_status,names='Comment_Status', values='ID', title='Comment status', color='Comment_Status', color_map={ 'Yes': minty_colors['primary'], 'No': minty_colors['secondary'] } )
    cp_pn_count = create_pie_chart(df_count_sentiment,names='Sentiment', values='count',title='Comment Sentiment', color='Sentiment',color_map={'Positive': minty_colors['positive'], 'Negative': minty_colors['negative'] } )
    cp_pn_count_gender = create_bar_chart(yes_count_gender_sentiment, x='Gender', y='count',title='Count number of sentiments by gender',x_label='Gender', y_label='# of surveys',color='Sentiment',color_map={ 'Positive': minty_colors['positive'], 'Negative': minty_colors['negative'] } )
    cp_pn_count_age = create_bar_chart(yes_count_age_sentiment, x='Age_Range', y='count', title='Count number of sentiments by age range', x_label='Age range', y_label='# of surveys',color='Sentiment', color_map={'Positive': minty_colors['positive'],'Negative': minty_colors['negative']})
    cp_stat_point = create_bar_chart_ng(yes_df_sum, x='index', y='Mean_Point',title='Statistics on the satisfaction points',x_label='Stat', y_label='Satisfaction point',color=minty_colors['accent'])
    cp_p_stat_point = create_bar_chart_ng(yes_df_sum, x='index', y='Positive_Score', title='Statistics on Positive Sentiment',x_label='Stat', y_label='Satisfaction point',color=minty_colors['primary'])
    cp_n_stat_point = create_bar_chart_ng(yes_df_sum, x='index', y='Negative_Score',title='Statistics on Negative Sentiment',x_label='Stat', y_label='Satisfaction point',color=minty_colors['secondary'])
    cp_compare_avg_point = create_bar_chart_ng(table_compare, x='Comment_Status', y='mean',title='Average of satisfaction points comparison',x_label='Status', y_label='Satisfaction point',color=minty_colors['accent'])
    cp_compare_std_point = create_bar_chart_ng(table_compare, x='Comment_Status', y='std',title='SD of satisfaction points comparison',x_label='Status', y_label='Satisfaction point',color=minty_colors['secondary'])

    # wp_word_cloud = html.Img(src='data:image/png;base64,{}'.format(create_word_cloud(df_words)))
    wp_word_count_fig = create_hbar_chart(top10, x=top10.index, y=top10.values,title='TOP 10 Words',x_label='', y_label='count',color=minty_colors['secondary'])
    return op_count_age_fig, wp_word_count_fig
'''(op_count_age, op_count_gender, op_stat_point, op_age_mean, op_gender_mean, 
            cp_have_count, cp_pn_count, cp_pn_count_gender, cp_pn_count_age, cp_stat_point, cp_p_stat_point, cp_n_stat_point, cp_compare_avg_point, cp_compare_std_point, 
            # wp_word_cloud, 
            wp_word_count)'''
